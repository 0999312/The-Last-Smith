	 ...
	 public void onStartupEmpowered(EntityPlayer player, ItemStack stack)
	  {
	    ...
	    if (energyUsage <= energy)
	    {
            for(int i = 0; i < 6;i++){
                ...
                entityDrive.setLocationAndAngles(player.posX,
                        player.posY + player.getEyeHeight()/2D,
                        player.posZ,
		...
		public void addInformation(ItemStack stack, World arg1, List tooltip, ITooltipFlag arg3) {
			...
			if(BladeUtil.getname(tag) != null){
				tooltip.add(TextFormatting.GOLD + I18n.format("blades.crafter")+":"+TextFormatting.GRAY+BladeUtil.getname(tag));	
			}
			...
			tooltip.add(StringHelper.RED +StringHelper.localize("info.flammpfeil.slashblade.tool.user") + ": " + BladeUtil.Username.get(ItemSlashBlade.getItemTagCompound(stack)));
