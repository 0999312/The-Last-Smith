...
public class ItemSlashBladeWind extends ItemSlashBladeNamedSS {
	    ...
	    if (player.world.isRemote)
	    {
	      ...
	      for (int a = 0; a < 5; a++) {
	        FXDispatcher.INSTANCE.smokeSpiral(player.posX, 
	          player.getEntityBoundingBox().minY + player.height / 2.0F, player.posZ, 1.5F, player.world.rand
	          .nextInt(360), miny, 14540253);
	      }
	      if (player.onGround)
	      {
	        ...
	        player.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, player.posX, 
	          player.getEntityBoundingBox().minY + 0.1000000014901161D, player.posZ, mx, 0.0D, mz, new int[0]);
	      }
	    }
