 ...
 */
public class RenderSA extends Render{

    static public WavefrontObject model = null;

    static public ResourceLocationRaw modelLocation = new ResourceLocationRaw("flammpfeil.slashblade","model/util/san.obj");
    static public ResourceLocationRaw textureLocation = new ResourceLocationRaw("flammpfeil.slashblade","model/util/slashdim.png");

    public RenderSA(RenderManager renderManager) {
        super(renderManager);
    }
    ...
    public void renderModel(Entity entity, double x, double y, double z, float yaw, float partialRenderTick) {
        ...
        if(entity instanceof EntitySA) {
            color = ((EntitySA) entity).getColor();
            lifetime = ((EntitySA)entity).getLifeTime();
        }
